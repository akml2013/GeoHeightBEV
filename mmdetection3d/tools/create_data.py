...
def kitti_dair_v2x_i_data_prep(root_path,
                    info_prefix,
                    version,
                    out_dir,
                    with_plane=False):
    """Prepare data related to dair_v2x_i dataset.
    ...
    Args:
        ...
        with_plane (bool, optional): Whether to use plane information.
            Default: False.
    """
    kitti.create_kitti_dair_v2x_i_info_file(root_path, info_prefix, with_plane)
    kitti.create_reduced_point_cloud(root_path, info_prefix)
    ...
    update_pkl_infos('kitti_dair_v2x_i', out_dir=out_dir, pkl_path=info_train_path)
    update_pkl_infos('kitti_dair_v2x_i', out_dir=out_dir, pkl_path=info_val_path)
    update_pkl_infos('kitti_dair_v2x_i', out_dir=out_dir, pkl_path=info_trainval_path)
    # update_pkl_infos('kitti_dair_v2x_i', out_dir=out_dir, pkl_path=info_test_path)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'kitti':
        if args.only_gt_database:
            ...
            if args.kitti_type == 'dair-v2x-i':
                print("dair-v2x-i")
                kitti_dair_v2x_i_data_prep(
                    root_path=args.root_path,
                    info_prefix=args.extra_tag,
                    version=args.version,
                    out_dir=args.out_dir,
                    with_plane=args.with_plane)
            if args.kitti_type == 'kitti':
                kitti_data_prep(
                    root_path=args.root_path,
                    info_prefix=args.extra_tag,
                    version=args.version,
