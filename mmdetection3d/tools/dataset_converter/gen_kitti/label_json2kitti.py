...
def write_kitti_in_txt(my_json, path_txt):
    ...
    for item in my_json:
        ...
        if isinstance(item["rotation"], str):
            adjusted_rotation = -eval(item["rotation"])+ np.pi/2
        else:
            adjusted_rotation = -item["rotation"] + np.pi/2

        if adjusted_rotation > np.pi:
            adjusted_rotation -= 2 * np.pi
        elif adjusted_rotation < -np.pi:
            adjusted_rotation += 2 * np.pi

        i15 = str(adjusted_rotation)
...
def rewrite_txt(path):
    with open(path, "r+") as f:
        ...
        find_str_motorcyclist = "Motorcyclist"
        replace_str_cyclist = "Cyclist"
        ...
        for line in data:
            ...
            if find_str_motorcyclist in line:
                line = line.replace(find_str_motorcyclist, replace_str_cyclist)
