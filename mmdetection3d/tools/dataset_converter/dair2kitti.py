...
from utils import pcd2bin, read_json
...
def mdkir_kitti(target_root):
    ...
    os.makedirs(os.path.join(target_root, "testing/calib"), exist_ok=True)
    os.makedirs(os.path.join(target_root, "testing/label_2"), exist_ok=True)
    os.makedirs(os.path.join(target_root, "testing/image_2"), exist_ok=True)
    os.makedirs(os.path.join(target_root, "testing/velodyne"), exist_ok=True)
...
def copy_test_files(target_root, ImageSets_path):
    ...
    dirs = ['image_2', 'velodyne', 'label_2', 'calib']
    for dir_name in dirs:
        src_dir = os.path.join(target_root, 'training', dir_name)
        dest_dir = os.path.join(target_root, 'testing', dir_name)
        os.makedirs(dest_dir, exist_ok=True)
        
        for file_id in test_files:
            if dir_name == 'image_2':
                ext = '.jpg'
            elif dir_name == 'velodyne':
                for ext in ['.bin', '.pcd']:
